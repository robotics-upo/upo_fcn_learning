...
Capture::Capture(sensor_msgs::PointCloud* obs, vector<upo_msgs::PersonPoseUPO>* people, geometry_msgs::PoseStamped* goal, vector<geometry_msgs::PoseStamped>* agent)
{
	setScenario(obs, people, goal, agent);
	setup();
...
void Capture::setScenario(sensor_msgs::PointCloud* obs, vector<upo_msgs::PersonPoseUPO>* people, geometry_msgs::PoseStamped* goal, vector<geometry_msgs::PoseStamped>* agent) {
...
*/
void Capture::generateImg(sensor_msgs::PointCloud* pc, vector<upo_msgs::PersonPoseUPO>* people, geometry_msgs::PoseStamped* goal, vector<geometry_msgs::PoseStamped>* path, string name)
{
	setScenario(pc, people, goal, path);
	generateImg(name);
}
...
* Generate and store the image
*/
void Capture::generateImg(string name)
{
	cv::Mat img;
	//if(use_static_map)
	//{
...
int main(int argc, char** argv){
	...
	ros::init(argc, argv, "-Capture imgs-");

	bool exit = false;
	while(n.ok() && !exit)
	{
		cout << endl << "Waiting for any key to record image... (type 'q' to exit)" << endl;
		char input;
		cin >> input;  //Waiting for an input from keyboard
		ros::spinOnce();
		if(input != 'q') {
			generateImg();
		}else{
